...
function icon(){
    return src(path.src.icons) // возвращаем html из src
    .pipe(dest(path.build.icons))// строит в папку  build html
    .pipe(browsersync.stream()) // обновляет браузер 
}
...
let build = gulp.series(clean,gulp.parallel(js,css,html,img,icon))
